...
export const SearchTrigger: React.FC<SearchTriggerProps> = ({
  ...
  // Hydration-safe shortcut display
  const [shortcutDisplay, setShortcutDisplay] = React.useState<'⌘K' | 'Ctrl+K'>('Ctrl+K');
  React.useEffect(() => {
    if (typeof navigator !== 'undefined' && navigator.platform.includes('Mac')) {
      setShortcutDisplay('⌘K');
    } else {
      setShortcutDisplay('Ctrl+K');
    }
  }, []);
  ...
  if (variant === 'icon') {
    return (
      <button
        ...
        title={`Open search (${shortcutDisplay})`}
      >
        <Search size={18} />
      </button>
    );
  ...
  if (variant === 'button') {
    return (
      ...
      >
        ...
        {showShortcut && (
          <span className="ml-2 text-xs text-muted font-mono bg-surface-light px-2 py-1 rounded">
            {shortcutDisplay}
          </span>
        )}
        {showShortcut && (
          <div className="ml-auto flex items-center gap-1 text-xs text-text-secondary">
            <Command size={10} />
            <span>{shortcutDisplay.slice(-1)}</span>
          </div>
        )}
      </button>
    );
